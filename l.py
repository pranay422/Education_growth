...
def update_mysql_table_animal(alphabet_name, is_correct):
    ...
    try:
        ...
        cursor.execute("""
            INSERT INTO alphabet_data (alphabet_name, attempt, correct, incorrect, timestamps, dates)
            VALUES (%s, 1, %s, %s, %s, %s)
        """, (alphabet_name, 1 if is_correct else 0, 0 if is_correct else 1, current_timestamp, current_date))
...
def login_page():
    ...
    if st.button("Log In"):
        if username and password:
            # Validate credentials (you can add a proper authentication mechanism here)
...
def load_data_from_mysql_alpha():
        try:
            ...
            query = "SELECT alphabet_name, attempt, correct, incorrect, timestamps, dates FROM alphabet_data;"
